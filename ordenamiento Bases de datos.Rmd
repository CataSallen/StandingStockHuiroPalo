...
Transect<-Station %>% group_by(Season, Site, Latitude, Transect) %>% summarise(meanTransect=mean(meanStation), seTransect=sd(meanStation)/sqrt(length(Transect)))
...
##Grafico variación latitudinal por temporada
...
transectsub2$Site <- gsub("Quintay sector A", "Quintay protected", transectsub2$Site)
...
transectsub19$Site <- gsub("Quintay sector A", "Quintay protected", transectsub19$Site)
...
ifop19$Site <- gsub("Quintay A", "Quintay protected", ifop19$Site)
...
condicion20= (ifop19$Site== "Quintay protected")
...
####Variación de la biomasa en un gradiente latitudinal y anual
...
herbTransect<-herbQuad %>% group_by(Site, Transect, Functional.Group) %>% summarise(AbH= mean(Abundances), BioKelp= mean(Biomasa))

 
ggplot(herbTransect, aes(x=AbH, y= BioKelp, colour= Site))+geom_point()
```

####Correlación entre Latitud y biomasa 

```{r}
Biomasa1213<-Station %>% mutate(Year=case_when(Season %in% c("Fall2012","Spring 2012") ~ "2012",
                                             Season %in% c("Fall2013","Spring 2013") ~ "2013")) %>% 
                        group_by(Year, Site, Latitude, Transect) %>% 
                        summarise(meanBTrans=mean(meanStation), seBTrans=sd(meanStation)/sqrt(length(meanStation)))
...
##Alguna de estas me arregló el problema de unir las df
Biomasa1213$Year<-as.factor(Biomasa1213$Year)
Biomasa1213$Transect<-as.character(Biomasa1213$Transect)
Biomasa1213<-as.data.frame(Biomasa1213)
BiomasaStat18$Transect<-as.character(BiomasaStat18$Transect)
BiomasaStat18$Year<-as.factor(BiomasaStat18$Year)
BiomasaStat18<-as.data.frame(BiomasaStat18)

#Unimos los datos del 2012, 2013 y 2018
Biomasa121318<-bind_rows(Biomasa1213,BiomasaStat18)

LatBiomass<-ggplot(Biomasa121318, aes(x=Latitude, y=meanBTrans, colour= Year)) + geom_point()
LatBiomass
```
