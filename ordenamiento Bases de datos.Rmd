...
```{r}
Alldata %>% group_by(Site,Season, Date) %>% summarise(Latitud= unique(Latitude), Longitude=unique(Longitude)) %>% arrange(Season)
 model<-lm(log(Diametro.extraida)~log(peso), data= Peso)
