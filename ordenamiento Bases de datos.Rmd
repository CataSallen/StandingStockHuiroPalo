...
```{r}
#DF de los transectos submareales 
#transectsub<-read.csv("transectos18.csv")
library(readxl)
transectsub<- read_xls("transectos18.xls") ##no me leyo el csv, lo lei con xls y lo transformé a csv  
write.csv(transectsub, "transectossub18.csv")

transectsub2<-read.csv("transectossub18.csv", sep = ",") %>% select(c(`Ano`,`AMERB`,`Transecto`,`Latitud.inicio`, `Longitud.inicio`)) %>% rename(Year= "Ano", Site= "AMERB", Transect="Transecto", Latitude= "Latitud.inicio", Longitude="Longitud.inicio")
...
##Definir coordenadas
## unir coordenadas de los transectos con datos de cuadrantes 

ifop_coordenadas<-left_join(ifop18,transectsub2)

###Aquí se puede ver que no todas las transectas tienen datos de las coordenadas, por lo que usaremos una coordenada por sitio.
###Coordenadas serán incluidas manualmente en la base de datos ifop18
...
unique(ifop19$Abundance) ##[1] 1  0   -, tenemos un - que debería ser un NA por que la abundancia no aplica a los datos de animales





#para finalmente tener la base de datos del 2018 y 2019
IFOP1819<- rbind(ifop18,ifop19)
unique(IFOP1819$Group)
unique(IFOP1819$Abundance)
unique(ifop18$Abundance)
unique(df$Abundance)


Ltrabeculata<-IFOP1819 %>% filter (Group=="Macroalga parda") %>% filter(Species !="Lessonia berteroana"& Species !="Macrocystis pyrifera") %>% mutate(Biomass=(0.0026)*((Size)^(2.12524))) %>%  mutate(Biomass=replace_na(Biomass, 0))
  
#Alldata2<-rbind(Alldata,IFOP1819)
#Alldata3<-rbind(Alldata,IFOP1819[-])
```


####Variación de la biomasa en un gradiente latitudinal
