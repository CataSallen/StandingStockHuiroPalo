...
Alldata$Biomass= (5.27993)*((Alldata$Size)^(2.12524))/1000##Esta es la correcta!
...
Ltrabeculata<-IFOP1819 %>% filter (Group=="Macroalga parda") %>% filter(Species !="Lessonia berteroana"& Species !="Macrocystis pyrifera") %>% mutate(Biomass=(5.27993)*((Size)^(2.12524)/1000)) %>% mutate(Biomass=replace_na(Biomass, 0)) %>%  mutate(Size= case_when(Abundance=="0"~ "0", TRUE ~ as.character(Size)), Stipes= case_when(Abundance=="0"~ "0", TRUE ~ as.character(Stipes)), Length= case_when(Abundance=="0"~ "0", TRUE ~ as.character(Length)))
...
### Biomasa 2019-NERC
```{r}
nerc19<-read.csv("KelpQuads.csv")%>% mutate(Biomass=(5.27993)*((Holdfast.Diameter)^(2.12524)/1000))
...
BiomasaNerc19<- Quadnerc19 %>% group_by(Site) %>% summarise(B= mean(Biomassm2, na.rm = T) , se= sd(Biomassm2, na.rm = T)/sqrt(length(Transect.Number)))

```

```{r plot Biomasa NERC19}
BiomasaNERC19plot<-ggplot(BiomasaNerc19, aes(x=fct_relevel(Site, "Isla Grande de Atacama","Puerto Viejo","Chanaral de Aceituno TURF","Chanaral de Aceituno OA","Los Mollies OA", "La Ballena TURF"), y=B)) + geom_bar(position = "dodge", stat = "identity", size =.2)+ 
...
histHoldfast<-ggplot(data = nerc19, aes(x =Holdfast.Diameter )) +
    geom_bar(breaks=seq(1,75,by=10), colour = "black", stat="count") + labs(x= "Diámetro del disco" ,y="Frecuencia")+theme_light()+facet_grid(.~Site)

hist<-ggplot(data = nerc19, aes(x =Holdfast.Diameter )) +
    geom_histogram(breaks=seq(1,60,by=5), colour = "red", fill = "green", stat="count") + theme_bw()+facet_grid(.~Site)
...
MORFO<-Quad18 %>% select(Size, Stipes, Length, Biomassm2) %>% filter(!Size== 0, Stipes!="NaN", Length!="NaN",Biomassm2!="NaN") 
pcaMORFO<-MORFO[,6:9]
