...
F11110351$Season <- gsub("summer2014", "Summer 2014", F11110351$Season)
F11110351$Species <- gsub("Lessoniatrabeculata", "Lessonia trabeculata", F11110351$Species)
...
F1140841$Season <- gsub("summer 2014", "Summer 2014", F1140841$Season)
...
F1151094$Year <- gsub("15", "2015", F1151094$Year)
F1151094$Year <- gsub("16", "2016", F1151094$Year)
F1151094$Year <- gsub("17", "2017", F1151094$Year)
...
#De otra manera ...promediamos todos los cuadrantes de todos los transectos
...
#Extraer latitudes por sitio ppara datos de temperatura
...
Herbivoros<-IFOP1819 %>% filter(Year=="2018", Species=="Tetrapygus niger" | Species =="Lessonia trabeculata" | Species=="Tegula tridentata" | Species=="Taliepus marginatus" | Species=="Loxechinus albus" | Species=="Taliepus sp" | Species== "Fissurella cumingii" | Species=="Tegula quadricostata" | Species=="FIssurella maxima" | Species== "Fissurella latimarginata" | Species=="Taliepus sp." | Species== "Taliepus dentatus"| Species=="Tegula atra" | Species=="Fissurella spp." | Species=="Fissurella costata") %>% mutate(Functional.Group= case_when(Species == "Lessonia trabeculata" ~ "Primary producers",
                                                                      TRUE ~  "Herbivore"), Biomasa=(0.0026)*((Size)^(2.12524))) %>% mutate(Biomasa=replace_na(Biomasa, 0)) %>% select(Site,Latitude, Transect, Replicate, Functional.Group, Species, Abundance, Biomasa)
